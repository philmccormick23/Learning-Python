...
def newPackage(request):
    ...
    return render(request,'newPackage.html')

def createPackages(request):
    Package.objects.create(cost=request.POST['cost'], name=request.POST['name'], user=User.objects.get(id=request.session['id']))
...
def update(request, id):
    x=Package.objects.get(id=id)
    x.name = request.POST['name']
    x.cost = request.POST['cost']
    x.save()
    return redirect('/packages')
